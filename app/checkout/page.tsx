...
export default function CheckoutPage() {
  ...
  const activeCoupon = getActivePublicCoupon()
                ...
                {couponApplied ? (
                  ...
                  <>
                    {activeCoupon && (
                      <button
                        type="button"
                        onClick={() => handleApplyCoupon(activeCoupon.code)}
                        disabled={couponLoading || subtotal < activeCoupon.minSubtotal}
                        className="mb-3 flex w-full items-center justify-between gap-3 rounded-xl border border-dashed border-[#12BC00]/50 bg-[#DCFDCC]/40 px-4 py-3 text-left transition-colors hover:bg-[#DCFDCC]/70 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                        <span className="flex items-center gap-2">
                          ...
                          <span className="text-sm leading-tight">
                            <span className="font-bold text-gray-900">{activeCoupon.code}</span>
                            <span className="ml-1.5 text-gray-600">{activeCoupon.label}</span>
                          </span>
                        </span>
                        <span className="shrink-0 text-sm font-semibold text-[#0fa600]">
                          {subtotal < activeCoupon.minSubtotal
                            ? `Add ₹${Math.ceil((activeCoupon.minSubtotal - subtotal) / 100)} more`
                            : couponLoading
                              ? 'Applying…'
                              : 'Apply'}
                        </span>
